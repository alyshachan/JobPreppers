...
export default function AddJobForm({ setJobs }) {
  ...
  const fetchJobs = async () => {
    try {
      const res = await fetch("http://jobpreppers.co:5000/api/jobpost");
      if (res.ok) {
  ...
  const parseDescription = async () => {
    try {
      const res = await fetch("http://jobpreppers.co:5000/api/textanalytics", {
        method: "POST",
  ...
  const onSubmit = async (data, e) => {
    ...
    try {
      ...
      const response = await fetch("http://jobpreppers.co:5000/api/jobpost/add", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
